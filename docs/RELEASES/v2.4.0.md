...
This release consolidates Sprint 7 (CLI completeness for Appendix F) onto
the v2.3.0 base. It resolves the long-standing gap between the academic
Appendix F catalog and the actual repository state by adding the six
high-value commands from section A.2 of Appendix F, together with a new
source-of-truth document distinguishing implemented commands from
documented design proposals.

## Highlights

### Appendix F gap resolved

`docs/cli_reference.md` is now the canonical CLI reference,
...
### Six new CLI commands (Appendix F § A.2)

| Appendix F | Command | Style |
|---|---|---|
...
**Why two commands are NEW analyses and not wrappers:** Appendix F § A.2.2
...
- **`--source bias_eval` / `multi_seed` for `generate-plots`** are
  ...
  do not yet exist. This matches the Appendix F spec without inventing
  data.
- **`--verify` for `prepare-datasets`** currently emits SHA-256 hashes
  only. A manifest file (`docs/datasets_manifest.json`) for full hash
  comparison is documented in Appendix F but not yet in the repo; the
...
- **Section B of Appendix F** is *documented design space*, not technical
...
- Six new commands available. See `puma <command> --help` for usage
  or read `docs/cli_reference.md` § A.2.
