...
|----|-------------|--------|--------------------|----------------|-------------------|
| D17 | `deepseek-r1` parse_failure_rate ≈ 0.8 with `triage_jira` scenario. The model is intrinsically reasoning-style and emits `<think>...</think>` blocks plus extended reasoning before the final label, which the current parser does not strip | Phase B.2.5 smoke | parse_failure 0.8 in 5-instance smoke; excluded from B.3 sweep | Extend scenario parser to strip `<think>...</think>` content before label match; TDD-first | After Phase B.4 analysis (deepseek-r1 excluded from current sweep) |
...
| D6 | Timestamp columns in ORM models use Python-side defaults, not database-side `server_default` | Phase A.1 | Add `server_default=func.now()` via Alembic migration; preserves consistency in concurrent inserts | Phase E |
| D1 | `--validate-baseline` CLI flag declared in earlier specifications but not implemented | Phase A.0 | Add subcommand to `puma` CLI that runs the canonical `baseline_triage.yaml` spec and asserts F1 within tolerance ±0.01 | Phase E |
...
| D10 | `pre-commit` not installed as a local git hook (incompatibility with cross-container development workflow) | Phase A.3 | Document host-only setup in `docs/CONTRIBUTING.md` with instructions to install `pre-commit` via `pipx` and run manually | Phase E |
...
| D13 | `scripts/download_datasets.py` is mis-named: it processes a local SQL zip dump, not downloads anything | Phase A.7 | Rename to `process_datasets.py` or `prepare_datasets.py`; update any references in `data/README.md` and CI workflows | Phase E |
...
| D18-cleanup | `pyproject.toml` has `version = "2.0.0-dev"` which causes the published wheel to be named `puma-2.0.0.dev0-py3-none-any.whl` instead of `puma-2.0.0-py3-none-any.whl` | Phase A.7 observation | Standardize release versioning workflow: version bump to clean string before tag creation, then bump to next dev string after tag | Phase E |

## Summary

- Closed methodological findings: **8**
- Open technical debt items: **15** (Critical: 3, Medium: 7, Low: 5; one of those marked `DECIDED-NO-ACTION`)
- Total items tracked across project lifecycle: **23**
