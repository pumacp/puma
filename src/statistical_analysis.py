...
def analyze_triage() -> dict:
    ...
    for label, m in stats_triage["per_class"].items():
        logger.info(
            f"  {label:10} - P: {m['precision']:.3f}, R: {m['recall']:.3f}, F1: {m['f1']:.3f}"
        )
...
def analyze_estimation() -> dict:
    ...
    errors = [abs(t - p) for t, p in zip(y_true, y_pred, strict=False)]
    mae = np.mean(errors)
...
def generate_summary_report(stats_triage: dict, stats_estimation: dict, wilcoxon: dict):
    ...
    report = {"triage": stats_triage, "estimation": stats_estimation, "wilcoxon_test": wilcoxon}
